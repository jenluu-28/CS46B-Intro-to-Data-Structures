...
public class MassAverager 
{
	...
	public float getMeanPlanetaryMass()
	{
		...
		//Initialize local variable for float sum
		float sum = 0;
		
		//Get the array of planets
		for (Planet current: Planet.getAll())
		{
			//Add mass to the sum, use + first 
			sum += current.getMass();
		}
		
		//return the average mass of the planets as a float
		return (float)(sum/Planet.getAll().length);
